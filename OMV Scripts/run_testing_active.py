...
def trainOneEpoch_CWR(model, x_test, y_test, features, labels_features, clust_batch_size, found_digit, cntr):
    ...
    for i in range(0, test_samples):
        ...
        real_cntr = cntr-clust_batch_size+1+i 
        ...
        prediction_vec[i] = model.label[np.argmax(y_pred_c)]
        ...
        if(real_cntr%model_batch_size==0 and real_cntr!=0): 
            for k in range(0, w):
                if(found_digit[k]!=0):
...
def trainOneEpoch_OL(model, x_test, y_test, features, labels_features, batch_size):
    ...
    for i in range(0, test_samples):
        ...
        prediction_vec[i] = model.label[np.argmax(y_pred)]
...
while 1:
    ...
    if (myClass.TRAINING_FLAG == 0 or myClass.TRAINING_FLAG == 1):
        ...
        if(cntr_batch == batch_size):
            ...
            pseudo_labels, predictions, err_clu, err_mod = trainOneEpoch_CWR(Model_OL, feature_batch, label_batch, ref_feat, labels_features, batch_size, found_digit, cntr)
