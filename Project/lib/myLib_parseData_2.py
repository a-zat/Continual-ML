...
def loadDataFromTxt(filename):
    ...
    DATASET_PATH = ROOT_PATH + '/Master_Thesis-main/Letters_application/Python/Letter_dataset/Clean_dataset/' + filename + ".txt"
...
def sanityCheckDataset(dataset):
    ...
    for i in range(len(dataset)):
        ...
        if(dummy != None):
            ...
            numbers_ary.append(dataset[i])
            counter_ary[len(numbers_ary)] += 1

    print(f'    The numbers found are:              {numbers_ary}')
    print(f'    And for each number the counter is: {counter_ary}')
