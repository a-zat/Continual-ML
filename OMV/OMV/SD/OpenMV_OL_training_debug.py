...
#pippo = [1.1, 10.123, 1.23, 0, 0.0,  200.345]
...
while(True):
    ...
    #img.midpoint(1, bias=0.85, threshold=True, offset=3, invert=True)
    ...
    for i in range(0, len(out_frozen)):  # [0].output())):
        if out_frozen[i] == 0.0: #if outputf[i] == 0.0:
            str1 = '0.000'
        elif out_frozen[i] >= 10:
            led1.toggle()
            ...
            str1 = '%.3f'%out_frozen[i]   #'%.3f'%outputf[i]
        strout = strout + str1 + ','

    '''

    strout = ''
    for i in range(0, len(pippo)):  # [0].output())):
        if pippo[i] == 0.0: #if outputf[i] == 0.0:
            str1 = '0.000'

        elif pippo[i] >= 10 and pippo[i] < 100:
            print(pippo[i])
            str1 = '%.2f'%pippo[i]
    ...
    if counter % 30 == 0:
        ...
        print('\n Length of the string is ', len(strout))
